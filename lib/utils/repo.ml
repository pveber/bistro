...
let build ?np ?mem ?loggers ?allowed_environments ?(bistro_dir = "_bistro") ?collect ~outdir repo =
  let db = Db.init_exn bistro_dir in
  let expressions = List.map repo ~f:(item_to_workflow) in
  let sched = Scheduler.create ?np ?mem ?loggers ?allowed_environments ?collect db in
...
let build_main ?np ?mem ?loggers ?allowed_environments ?bistro_dir ?collect ~outdir repo =
  build ?np ?mem ?loggers ?allowed_environments ?bistro_dir ?collect ~outdir repo
  |> Lwt_main.run
